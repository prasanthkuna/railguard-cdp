...
type PolicyResult = "allow" | "block" | "escalate";
type InvoiceStatus =
  | "received"
  | "ready"
  | "needs_approval"
  ...
  async (params: CreatePaymentIntentRequest): Promise<{ paymentIntent: PaymentIntent }> => {
    ...
    const row = await db.queryRow<PaymentIntentRow>`
      ...
      VALUES (
        ...
        ${BASE_SEPOLIA_USDC}, ${invoice.walletAddress}, ${invoice.amountBaseUnits},
    ...
    await appendAudit("payment_intent", paymentIntent.id, "payment_intent.created", { paymentIntent });
    await appendAudit("invoice", invoice.id, "payment_intent.created", { paymentIntentID: paymentIntent.id });
...
export const executePaymentIntent = api(
  { expose: true, auth: true, method: "POST", path: "/payment-intents/:id/execute", sensitive: true },
  async (params: ExecutePaymentIntentRequest): Promise<{ paymentIntent: PaymentIntent }> => {
    ...
    const txHash = `0x${sha256(`${existing.id}:${params.idempotencyKey}`).slice(0, 64)}`;
    const row = await db.queryRow<PaymentIntentRow>`
    ...
    await appendAudit("invoice", invoice.id, "payment_intent.executed", { paymentIntentID: paymentIntent.id, txHash });
    return { paymentIntent };
  },
);
...
export const getAuditTrail = api(
  ...
  async ({ entityType, entityID }: { entityType: string; entityID: string }): Promise<{ auditEvents: AuditEvent[] }> => ({
...
async function runPolicy(invoice: Invoice): Promise<PolicyRun> {
  ...
  const amount = ensurePositiveBaseUnits(invoice.amountBaseUnits);
  const rules: string[] = [];
  ...
  if (vendor.status === "blocked") rules.push("vendor.blocked");
  if (vendor.riskScore >= 80) rules.push("vendor.risk_score_block");
  if (normalize(invoice.token) !== "usdc") rules.push("invoice.unsupported_token");
  if (normalize(invoice.chain) !== "base-sepolia") rules.push("invoice.unsupported_chain");
  if (!isAddress(invoice.walletAddress)) rules.push("wallet.invalid_address");
  if (amount > HARD_CAP_BASE_UNITS) rules.push("amount.hard_cap");
  if (duplicate) rules.push("invoice.duplicate");

  const exactWallet = approvedWallets.find(
    (wallet) => normalize(wallet.chain) === normalize(invoice.chain) && wallet.address.toLowerCase() === invoice.walletAddress.toLowerCase(),
  );
  ...
  if (vendor.status === "pending") escalations.push("vendor.pending_onboarding");
  if (!invoice.invoiceNumber) escalations.push("invoice.missing_number");
  if (invoice.extractionConfidence < 0.8) escalations.push("invoice.low_extraction_confidence");
  if (amount > REVIEW_THRESHOLD_BASE_UNITS) escalations.push("amount.requires_review");

  const result: PolicyResult = rules.length > 0 ? "block" : escalations.length > 0 ? "escalate" : "allow";
  const allRules = [...rules, ...escalations];
  ...
  const row = await db.queryRow<PolicyRunRow>`
    ...
    VALUES (${policyID}, ${actor.organizationID}, ${invoice.id}, ${result}, ${JSON.stringify(allRules)}, ${JSON.stringify(evidence)})
    RETURNING *
  `;
...
async function appendAudit(
  ...
  const eventHash = sha256(stableStringify({ eventID, entityType, entityID, eventType, event, previousHash }));
...
function requireActor(allowedRoles?: Array<"owner" | "finance" | "approver" | "viewer">) {
  const actor = getAuthData();
...
async function findDuplicateInvoice(organizationID: string, invoice: Invoice): Promise<Invoice | null> {
  const row = await db.queryRow<InvoiceRow>`
    SELECT * FROM invoices
...
async function latestPolicyRun(organizationID: string, invoiceID: string): Promise<PolicyRun | undefined> {
  const row = await db.queryRow<PolicyRunRow>`
...
function ensureAddress(value: string): void {
  if (!isAddress(value)) throw APIError.invalidArgument("wallet address must be a 0x-prefixed EVM address");
}
