...
describe("X402Guard", () => {
    ...
    it("withSpendingPolicy throws before callback on block", async () => {
        const guard = new X402Guard({ policy: defaultDevPolicy("agent_demo") });
        const pay = withSpendingPolicy(() => true, guard, (amount, url) => ({
            agentId: "agent_demo",
            payer: "0x1111111111111111111111111111111111111111",
            payTo: "0x2222222222222222222222222222222222222222",
            amountAtomic: amount,
        ...
        await expect(pay(9000000n, "https://api.example.com/v1/data")).rejects.toBeInstanceOf(PolicyViolationError);
    });
});
