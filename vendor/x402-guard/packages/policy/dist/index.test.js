...
import { SpendTracker, buildPaymentFingerprint, defaultDevPolicy, evaluateAgentPolicy, } from "./index.js";
const baseCtx = () => ({
    agentId: "agent_demo",
...
describe("evaluateAgentPolicy", () => {
    ...
    it("blocks per-call cap", () => {
        const policy = defaultDevPolicy("agent_demo");
        const ctx = { ...baseCtx(), amountAtomic: 2000000n };
    ...
    it("escalates when mandate missing for large payment", () => {
        const policy = defaultDevPolicy("agent_demo");
        const ctx = { ...baseCtx(), amountAtomic: 600000n };
        const result = evaluateAgentPolicy(ctx, policy, new SpendTracker());
