...
export async function authorizePayment(store, input) {
    ...
    const reserved = await store.reserveBudget(input.ctx.agentId, input.ctx.amountAtomic, input.policy.windows, authorizationId, nowMs);
    if (!reserved) {
        return {
            ok: false,
            triggeredRules: input.policy.windows.map((window) => `budget.window_${window.windowSeconds}s_exceeded`),
            decision: "block",
            domain,
        };
    }
